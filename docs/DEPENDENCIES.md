...
|---|---|---|---|
| Android Gradle Plugin | 9.2.0 | Verified | Requires Gradle 9.4.1+, JDK 17, SDK Build Tools 36.0.0+, max API 37. `build-logic/convention`'s `ConfigureKotlinAndroid.kt`/`ConfigureAndroidCompose.kt` are written against AGP 9.2's real (non-generic) `CommonExtension` DSL — see `docs/DECISIONS.md`. `gradle.properties` (both root and `build-logic/`) sets `android.builtInKotlin=false` (unrelated runtime concern — keeps the classic KGP path this repo already applies explicitly) |
